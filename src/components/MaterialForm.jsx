...
function MaterialForm() {
  ...
  // Fetch materials from Firestore on mount
  useEffect(() => {
  ...
  // New states for district and soil info
  ...
  // Update soilInfo when selectedDistrict changes
  useEffect(() => {
    if (selectedDistrict && districts[selectedDistrict]) {
      setSoilInfo(districts[selectedDistrict]);
    } else {
      setSoilInfo(null);
    }
            ...
            >
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  {/* Soil Info Display */}
                  {soilInfo && (
                    <div className="mt-4 p-6 bg-white/30 backdrop-blur-md rounded-xl border border-green-300 shadow-sm">
                      <h5 className="text-lg font-semibold text-green-700 mb-4">
                        Soil Information
                      </h5>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-green-800 text-sm">
                        {Object.entries(soilInfo).map(([key, value]) => (
                          <div key={key} className="flex flex-col">
                            <span className="font-medium capitalize">{key.replace(/_/g, ' ')}</span>
                            <span className="mt-1">{String(value)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
